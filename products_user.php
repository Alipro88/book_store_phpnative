...
if(isset($_POST['add_to_cart'])){
   ...
   $select_cart = mysqli_query($conn, "SELECT * FROM `cart` WHERE  user_id = '$d' AND name =  '$product_name' ");

   if(mysqli_num_rows($select_cart) > 0){
      $message[] = 'product already added to cart';
   }else{
      $insert_product = mysqli_query($conn, "INSERT INTO `cart`(name, price, image, quantity ,user_id ) VALUES('$product_name', '$product_price', '$product_image', '$product_quantity','$d')");
...
<form action="" method="post">
         <div class="card p-4 mx-auto" style="width: 18rem; width: 250px; margin-top: 20px;">
            ...
            <div  clas=".card-body">
               ...
               <input style="background-color:white;" type="hidden" name="product_price" value="<?php echo $fetch_re['price']; ?> ">
               <input type="hidden" name="product_image" value="<?php echo $fetch_re['image']; ?>">
               <div class="price">$<?php echo $fetch_re['price']; ?>/-</div>
